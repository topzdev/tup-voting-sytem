...
const create = (_user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let user = user_entity_1.User.create({
        ...
        role: _user.role,
        password: yield (0, password_helper_1.genPassword)(user_helper_1.default.generatePassword(_user.username, _user.lastname)),
    });
    console.log(user);
...
const resetPassword = (_id) => __awaiter(void 0, void 0, void 0, function* () {
    if (!_id)
        throw new http_exception_1.HttpException("BAD_REQUEST", `user id is required`);
    const user = yield user_entity_1.User.findOne(_id);
    if (!user)
        throw new http_exception_1.HttpException("NOT_FOUND", "user not found");
    user.password = yield (0, password_helper_1.genPassword)(user_helper_1.default.generatePassword(user.username, user.lastname));
    yield user.save();
    return true;
});
const changePassword = (_passwords) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!user)
        throw new http_exception_1.HttpException("NOT_FOUND", "user not found");
    if (!(yield (0, password_helper_1.validatePassword)(_passwords.currentPassword, user.password)))
        throw new http_exception_1.HttpException("BAD_REQUEST", "password doesn't match");
    if (_passwords.confirmPassword !== _passwords.newPassword)
        throw new http_exception_1.HttpException("BAD_REQUEST", "current password doesnt match with new password");
    user.password = yield (0, password_helper_1.genPassword)(_passwords.newPassword);
